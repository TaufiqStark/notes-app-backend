...
class UsersService {
  ...
  async verifyUserCredential(username, password) {
    ...
    if (!result.rowCount) throw new AuthenticationError('Kredential yang Anda berikan salah');

    const { id, password: hashedPassword } = result.rows[0];

    const match = await bcrypt.compare(password, hashedPassword);

    if (!match) throw new AuthenticationError('Kredential yang Anda berikan salah');
